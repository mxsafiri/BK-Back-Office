...
import { type Money, tzs, subtract, add, gte } from "../../shared/money.js";
import { InsufficientFundsError } from "../../shared/errors.js";
...
export class InMemoryCashLedger implements CashLedger {
  ...
  async deposit(params: { userId: string; amount: Money }): Promise<CashRef> {
  ...
  async withdraw(params: { userId: string; amount: Money }): Promise<CashRef> {
    const bal = await this.getBalance(params.userId);
